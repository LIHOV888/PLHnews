...
// Error handling for layout rendering
app.use((err, req, res, next) => {
  if (err && err.message && err.message.includes('layout')) {
...
const mongoURI = process.env.MONGO_URI || 'mongodb+srv://db_user:REDACTED@hnews.mongodb.net/?appName=HnewsDB';
...
// User validation middleware - check if user still exists in DB
  ...
  if (req.user && req.user.id) {
    try {
      ...
      if (!user || !user.isActive || user.isBanned) {
        // User not found, inactive, or banned - destroy session
        req.session.destroy((err) => {
          if (err) {
            console.error('Session destruction error:', err);
          }
          // Redirect to login if accessing protected route
          if (req.path !== '/login' && req.path !== '/signup' && req.path !== '/' && !req.path.startsWith('/auth/')) {
      ...
      // DB error - allow request to continue with current session
...
// Override the layout rendering middleware
app.use((req, res, next) => {
  const originalRender = res.render;
  res.render = function(view, options, callback) {
    options = options || {};
    
    // Set layout based on route and ensure options are properly initialized
    options.layout = req.path.startsWith('/admin') ? 'admin/layout' : 'layout';
    options.title = options.title || (req.path.startsWith('/admin') ? 'Admin Panel' : 'Hnews');
    options.user = req.user || null;
    options.currentPath = req.path;
    options.analytics = options.analytics || null;
